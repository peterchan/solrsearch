...
public class ServerManage {
	...
	static {
		...
		InputStream resourceAsStream = ServerManage.class
				.getResourceAsStream(CONFIG_NAME);
		if (resourceAsStream == null) {
			resourceAsStream = ServerManage.class
		...
		serverUrl = (serverUrl.lastIndexOf(HTTP_PATH_SEPARATOR) != -1 ? serverUrl
				: serverUrl + HTTP_PATH_SEPARATOR);
	...
	public static HttpSolrServer getHttpServer(CoreNames core) {
		...
		httpServerCoreMap.put(core,
				new HttpSolrServer(serverUrl + core.getCoreName()));
		return httpServerCoreMap.get(core);
	}
	...
	public static SolrServer getConCurrentServer(CoreNames core) {
		...
		ConcurrentUpdateSolrServerMap.put(core, new ConcurrentUpdateSolrServer(
				serverUrl + core.getCoreName(), 1000, 3));
		return ConcurrentUpdateSolrServerMap.get(core);
